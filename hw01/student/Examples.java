...
public class Examples {
    ...
    /**
     * This method clears bit 4 (from the right; the rightmost bit is bit 0) of
     * the bit vector declared above. This is very similar to the clear method
     * you will have to implement in the BitVector.java file, and the general
     * approach should help you to figure out some of the other problems.
     *
    ...
    public void clearBitFourOfBitVector() {
        ...
        */
        bitVector = bitVector & mask; // clears bit 4 of the bit vector
    }
    ...
    /*
        If you want to convert a number entirely to octal, the algorithm is very 
        common/famous and is one the best ways.

        Why do we use a do-while loop?
        In the case that our input number is 0, then we still want to run 
        the loop once to ensure that the octal string is not empty. 
        Alternatively, we could use a conventional loop with an additional 
        if statement after the loop is over.
    */
    public String intToOctalString(int number)
    {
        String result = "";
        do {
            int rightmostOctalDigit = number & 0b111;
            //Note: 48 is the ASCII value for '0'
            result = (char)(rightmostOctalDigit + 48) + result;
            number = number >> 3;
        } while (number != 0);
        return result;
    }
